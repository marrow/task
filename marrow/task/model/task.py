...
class Task(Document):
	...
	def acquire(self):
		...
		count = self.__class__.objects(id=self.id, _owner=None, _acquired=None).update(
	...
	def release(self, force=False):
		...
		count = self.__class__.objects(id=self.id, _owner=identity).update(
				set___owner = None,
				set___acquired = None
			)
	...
	def execute(self):
		...
		ident = str(self.id)
		log.info("Attempting to execute task %s: %s", ident, self._callable)
		
		now = datetime.utcnow().replace(tzinfo=utc)
		
		self = self.acquire()
		if not self:
			log.warn("Unable to acquire task %s for execution.", ident)
			return
